...
class NumEscrito {
	...
	function saida($numero,$caps)
	{
		...
		$centavos = $separa[1];
		$strReais = ($numero > 1)? ' reais':' real';
		$saida = $this->escrever($numero). $strReais;
		
		if($centavos && $centavos > 0)
		{
			 $strCent = ($centavos > 1)?' centavos.':' centavo.';
			 $centavos = ' e '.$this->escrever($centavos) . $strCent;
			 $saida .= $centavos;
		}
