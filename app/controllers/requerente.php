...
class Requerente extends CI_Controller {
	var $usuario = '';
	...
	{		
		...
		$dados = $this->geral->requerente();
		//retorna os arquivo de um pagina
		$retorno = $this->paginator->separa_pag($dados,25,$pg);
		//retorna a paginação
		$link = base_url('requerente/lista/');
		$paginacao = $this->paginator->paginacao($retorno['numPg'],$link,$pg);
				
		$data['paginacao'] = $paginacao;
		$data['dados'] = $retorno['arquivoPg'];
		$data['paginacao'] = $paginacao;
		$this->template->load('template_view', 'requerente_list_view',$data);
	}
	function form_add()
	{
		$this->template->load('template_view', 'requerente_form_add_view',$data);
	}
	function add()
	{
		$set = $this->input->post();
		$this->klcrud->auto_add('cmr_requerente',$set,NULL,NULL);
		redirect(base_url('requerente/lista'));
	}
	function form_up($id)
	{
		$data['row'] = $this->geral->requerente($id);
		$this->template->load('template_view', 'requerente_form_up_view',$data);
	}
	function up($id)
	{
		$imputCheck = array('fl_scpc','fl_cerasa','fl_negativa','fl_vista','fl_prazo','fl_duplicata','fl_cheque','fl_cartao');
		$set = $this->input->post();
		$this->klcrud->auto_up('cmr_requerente',$set,array('cd_requerente'=>$id),NULL,$imputCheck);
		$this->klcrud->up('cmr_requerente',$imputCheck,array('cd_requerente'=>$id));
		redirect(base_url('requerente/lista'));
	}
	function procura_requerente()
	{
		...
		$this->load->view('requerente_search_view',$data);
	}
	function verifica_registro($tabela = 'cmr_requerente')
	{
	...
	function del($id)
	{
		...
		$processo = $this->db->get_where('cmr_processo',array('cd_requerente'=>$id))->result();
		...
		$this->db->delete('cmr_processo',array('cd_requerente'=>$id));
		$this->db->delete('cmr_acesso',array('cd_requerente'=>$id));
		$this->db->delete('cmr_requerente',array('cd_requerente'=>$id));
		redirect(base_url('requerente/lista'));
	}
}
